...
import { courses } from '../app/data/courses.data';
...
async function main() {
  ...
  // Create categories
  const categoryNames = [...new Set(courses.map(c => c.category))];
  ...
  for (const courseData of courses) {
    ...
    let course = await prisma.course.findUnique({ where: { title: courseData.title } });
